def game():
    ...
    def inputs():
        global Player1, Player2, p1_score, p2_score
        ...
        default()
        if Edit == "y":
            p1_stats_base['hp'] = input("> " +str(Player1)+ " hp? (d) for default")
            if p1_stats_base['hp'] == "d":
                p1_stats_base['hp'] = +int(stat1[1])
            print ("\n")
            p1_stats_base['attack'] = input("> "+str(Player1)+ " at? (d) for default")
            if p1_stats_base['attack'] == "d":
                p1_stats_base['attack'] = +int(stat1[2])
            print ("\n")
            p1_stats_base['defense'] = input("> "+str(Player1)+ " df? (d) for default")
            if p1_stats_base['defense'] == "d":
                p1_stats_base['defense'] = +int(stat1[3])
            print ("\n")
            p1_stats_base['speed'] = input("> "+str(Player1)+ " spd? (d) for default")
            if p1_stats_base['speed'] == "d":
                p1_stats_base['speed'] = +int(stat1[2])
            print ("\n")
            p2_stats_base['hp'] = input("> "+str(Player2)+ " hp? (d) for default")
            if p2_stats_base['hp'] == "d":
                p2_stats_base['hp'] = +int(stat2[1])
            print ("\n")
            p2_stats_base['attack'] = input("> "+str(Player2)+ " at? (d) for default")
            if p2_stats_base['attack'] == "d":
                p2_stats_base['attack'] = +int(stat2[2])
            print ("\n")
            p2_stats_base['defense'] = input("> "+str(Player2)+ " df? (d) for default")
            if p2_stats_base['defense'] == "d":
                p2_stats_base['defense'] = +int(stat2[3])
            print ("\n")
            p2_stats_base['speed'] = input("> "+str(Player2)+ " spd? (d) for default")
            if p2_stats_base['speed'] == "d":
